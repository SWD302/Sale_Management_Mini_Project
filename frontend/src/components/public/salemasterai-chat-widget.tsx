...
/**
 * Câu trả lời tĩnh cho các câu hỏi gợi ý.
 * Không gọi backend — trả lời ngay lập tức, không bao giờ bị lỗi 503.
 */
const STATIC_ANSWERS: Record<string, string> = {
  "SaleMaster có những tính năng gì?": `**SaleMaster VN** là nền tảng quản lý bán lẻ toàn diện với các tính năng nổi bật:

📦 **Quản lý sản phẩm** — Thêm, sửa, xoá sản phẩm; quản lý danh mục, biến thể, tồn kho theo thời gian thực.

🛒 **Quản lý đơn hàng** — Tạo đơn nhanh tại quầy (POS), theo dõi trạng thái đơn hàng, in hoá đơn.

👥 **Quản lý khách hàng** — Lưu thông tin khách, lịch sử mua hàng, chương trình tích điểm.

📊 **Báo cáo & thống kê** — Doanh thu, lợi nhuận, hàng bán chạy, tồn kho theo ngày/tháng/năm.

🔐 **Phân quyền người dùng** — Quản lý nhiều tài khoản với vai trò Admin, Nhân viên, Thu ngân.

🤖 **AI Agent tích hợp** — Hỗ trợ tư vấn và trả lời câu hỏi về hệ thống ngay trong giao diện.`,

  "Phù hợp loại hình kinh doanh nào?": `**SaleMaster VN** phù hợp với nhiều mô hình kinh doanh bán lẻ:

🏪 **Cửa hàng tạp hoá / tiện lợi** — Quản lý hàng hoá đa dạng, bán hàng nhanh tại quầy.

👗 **Shop thời trang / phụ kiện** — Quản lý biến thể (size, màu), tồn kho nhiều mẫu mã.

🍜 **Quán ăn / Cafe** — Tạo đơn nhanh, in bill, quản lý bàn và doanh thu ca làm việc.

💊 **Nhà thuốc / Hiệu thuốc** — Quản lý hạn sử dụng, danh mục thuốc, xuất nhập tồn kho.

📱 **Cửa hàng điện tử / di động** — Quản lý IMEI, bảo hành, sản phẩm giá trị cao.

Phù hợp nhất với **quy mô vừa và nhỏ (SMB)** từ 1 đến vài chục nhân viên.`,

  "Làm sao để bắt đầu dùng thử?": `Bạn có thể bắt đầu trải nghiệm **SaleMaster VN** theo các bước sau:

1️⃣ **Đăng ký tài khoản** — Tạo tài khoản Admin miễn phí tại trang đăng ký.

2️⃣ **Thiết lập cửa hàng** — Nhập thông tin cửa hàng, logo, địa chỉ và đơn vị tiền tệ.

3️⃣ **Thêm sản phẩm** — Import danh sách sản phẩm hoặc thêm thủ công từng mặt hàng.

4️⃣ **Tạo tài khoản nhân viên** — Phân quyền cho nhân viên bán hàng, thu ngân, kho.

5️⃣ **Bắt đầu bán hàng** — Dùng giao diện POS để tạo đơn và thanh toán ngay.

💡 **Thời gian thiết lập ban đầu** thường chỉ mất 15–30 phút. Liên hệ đội hỗ trợ nếu cần hướng dẫn thêm!`,

  "Hệ thống phân quyền hoạt động như thế nào?": `**SaleMaster VN** có hệ thống phân quyền theo vai trò (Role-Based Access Control):

👑 **Admin (Quản trị viên)** — Toàn quyền: quản lý sản phẩm, đơn hàng, nhân viên, báo cáo, cấu hình hệ thống.

🧑‍💼 **Manager (Quản lý)** — Xem báo cáo, quản lý sản phẩm và đơn hàng, không thay đổi cấu hình hệ thống.

🧑‍🔧 **Staff (Nhân viên / Thu ngân)** — Chỉ tạo và xử lý đơn hàng, xem sản phẩm, không thấy báo cáo tài chính.

🔑 **Mỗi tài khoản** đăng nhập bằng email + mật khẩu riêng, bảo mật JWT token.

✅ Admin có thể tạo, sửa, xoá tài khoản nhân viên bất kỳ lúc nào từ trang Quản lý người dùng.`,
};
...
export function SaleMasterAIChatWidget() {
  ...
  async function send(text: string) {
    ...
    // Đánh dấu câu hỏi gợi ý đã được dùng
    if (QUICK_QUESTIONS.includes(trimmed)) {
      setUsedQuestions((prev) => new Set([...prev, trimmed]));
    }
    ...
    // ── Kiểm tra câu trả lời tĩnh trước (không cần backend) ──
    const staticReply = STATIC_ANSWERS[trimmed];
    if (staticReply) {
      // Giả lập typing indicator ngắn cho UX tự nhiên hơn
      setLoading(true);
      await new Promise((r) => setTimeout(r, 600));
      setMessages((prev) => [...prev, { id: uid(), role: "assistant", text: staticReply }]);
      setLoading(false);
      return;
    }

    // ── Câu hỏi tự do → gọi backend ──
    setLoading(true);
    try {
        ...
        >
          ...
          <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-3 py-3">
            <div className="flex flex-col gap-3">
              ...
              {/* Quick questions — hiện sau mỗi câu trả lời của AI, loại bỏ câu đã hỏi */}
              {(() => {
                const remainingQuestions = QUICK_QUESTIONS.filter((q) => !usedQuestions.has(q));
                ...
                return (
                  <div className="flex flex-col gap-1.5 pl-9">
                    <p className="mb-0.5 text-[10px] text-muted-foreground">💡 Câu hỏi gợi ý:</p>
                    {remainingQuestions.map((q) => (
                      <button
                        key={q}
                        type="button"
                        onClick={() => void send(q)}
                        className="w-full rounded-xl border border-primary/20 bg-primary/5 px-3 py-2 text-left text-xs text-primary transition hover:bg-primary/10 hover:border-primary/40"
                      >
                        {q}
                      </button>
                    ))}
